...
回溯不计算路径虚实
'''
import numpy as np  # np版本为 1.26.4 时不会报错
import warnings
...
# ** 本程序在终端中使用的方法 ** #
# !! (注意outpath必须以/作为结尾) (id必须是数字且不超五位数，因其有内部建图意义) !! #
# !! 防止数据集名和id不符合要求，因此统一规定：以数据集开始的序号为id，同时为fasta命名 !! #
# python DAG_PHMM.py xxx.fasta /outpath/ id ilow iup mlow mup                                        # 标准格式
# *outpath示例：/home/handsomekk/repfam/fto/5/xxx_random/5-1/deep/                                    # 本地30
# nohup python3 DAG_PHMM.py /home/handsomekk/repfam/fto/5.fasta /*/ 5 ? ? ? ? > ./?_?.log 2>&1 &     # 本地30格式，测试5条
# fastapath = /disk2/kzw/try/90.fasta    # 71
# fastapath = /data1/kzw/try/90.fasta    # 187



# ctrl+r手动搜索替换。
# 2 3 4 5 6  ,  8 9 10 11 12  # 一共24个位置需要改random数字
# 不过现在已经全换成global_random_low/up了，直接在命令行里传递这些参数

# 随机扰动方式
# 可选：初始参数ini_pc的随机，训练更新后的mstep随机
